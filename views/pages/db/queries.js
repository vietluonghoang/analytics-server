...
const redeemAdsOptoutCoupon = (request, response) => {
  ...
  if (!valid) {
    ...
    pool.query('select case when count(c.coupon_code) < c.quantity then 1 else 0 end as isValid from coupons as c join ads_optout as a on c.coupon_code = a.last_redeemed_code where c.coupon_code = $1 and c.start_time >= $2 and c.end_time <= $2 group by c.coupon_code,c.quantity', [couponCodeLowercase, timestamp], (error, results) => {
      if (error) {
        throw error
      }
      //if result is 1 then there is still available coupon (0 is not)
      console.log('check available coupon result: ', results);
      console.log('check available coupon result isValid: ', results.isValid);
